public class Solution3 {
    ...
    public ListNode removeElements(ListNode head, int val) {
        ...
        while (prev.next != null) {
            ...
            // 如 [1 -> 6 -> 7]      链表中删除 6 的 prev 的引用变化为: prev(-1) [else 逻辑修改 prev 引用] -> prev(1) [if 逻辑执行删除] -> prev(2) [else 逻辑修改 prev 引用] -> [结束循环]
            // 如 [1 -> 6 -> 6 -> 7] 链表中删除 6 的 prev 的引用变化为: prev(-1) [else 逻辑修改 prev 引用] -> prev(1) [if 逻辑执行删除] -> prev(1) [if 逻辑执行删除] -> prev(2) [else 逻辑修改 prev 引用] -> [结束循环]
