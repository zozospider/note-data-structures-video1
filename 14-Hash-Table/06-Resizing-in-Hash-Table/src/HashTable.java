...
public class HashTable<K extends Comparable<K>, V> {
    ...
    public void add(K key, V value) {
        ...
        if (map.containsKey(key)) {
            ...
            // 如果哈希表中的元素个数大于等于 (数组长度 * 上界), 说明平均每个 map 存储的元素个数超过了上届, 将数组的容量扩容为原来的 2 倍
            if (size >= UPPER_TOLERANCE * maps.length) {
                resize(2 * maps.length);
            }
        }
    }
    ...
    public V remove(K key) {
        ...
        // 增加 (maps.length / 2 >= INIT_CAPACITY) 的条件判断用于防止当 (maps.length < 2 * INIT_CAPACITY) 时 (newCapacity < INIT_CAPACITY) 的情况
        if (size < LOWER_TOLERANCE * maps.length
                && maps.length / 2 >= INIT_CAPACITY) {
